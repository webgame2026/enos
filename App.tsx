...
const App: React.FC = () => {
  ...
  // Persistent Level Loading
  const [levels, setLevels] = useState<GameLevel[]>(() => {
    try {
      const saved = localStorage.getItem('mindwhack_levels');
      if (saved) {
        const parsed = JSON.parse(saved);
        // Ensure system levels are always present but not duplicated
        const userLevels = parsed.filter((l: GameLevel) => l.author !== 'System');
        return [...INITIAL_LEVELS, ...userLevels];
      }
  ...
  const handleLogout = () => {
    ...
    try {
      ...
      localStorage.removeItem('mindwhack_levels');
    } catch (e) {}
  };
